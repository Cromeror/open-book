...
```
apps/web/src/app/(dashboard)/account-statements/
...
|--------|-------|---------|------|
| /account-statements | requireModule | account_statements | Ver mi propio estado |
| /account-statements/settings | requirePermission | settings:update | Cambio via PQR |
| /account-statements/public | Custom | - | Solo si soy publico |

## Logica Especial

### /account-statements/public
...
const myVisibility = await getUserVisibility(permissions.userId);

if (myVisibility !== 'PUBLIC') {
  redirect('/account-statements'); // No puede ver estados publicos
}
```
