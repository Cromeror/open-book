...
describe('ModuleRegistry', () => {
  ...
  it('should check module access', () => {
    moduleRegistry.setUserModules([
      createModule('goals', [{ code: 'read', settings: { type: 'read' } }], { path: '/goals', order: 1 })
    ]);

    expect(moduleRegistry.hasModule('goals')).toBe(true);
    expect(moduleRegistry.hasModule('contributions')).toBe(false);
  });
  ...
  it('should check actions (codigo accion = codigo permiso)', () => {
    ...
    expect(moduleRegistry.hasAction('goals', 'read')).toBe(true);
    expect(moduleRegistry.hasAction('goals', 'create')).toBe(true);
    expect(moduleRegistry.hasAction('goals', 'delete')).toBe(false);
  });

  it('should get action settings typed', () => {
    const readSettings = { type: 'read', listColumns: [{ field: 'name', label: 'Nombre' }] };
    moduleRegistry.setUserModules([
      createModule('goals', [{ code: 'read', settings: readSettings }], { path: '/goals', order: 1 })
    ]);

    const settings = moduleRegistry.getActionSettings<ReadActionSettings>('goals', 'read');
  ...
  it('should return specialized component', () => {
    ...
    moduleRegistry.register('reports', MockComponent);
    moduleRegistry.setUserModules([{
      code: 'reports',
      label: 'Reportes',
      icon: 'BarChart3',
    ...
    const component = moduleRegistry.getSpecializedComponent('reports');
  ...
  it('should return null for CRUD modules', () => {
    ...
    const component = moduleRegistry.getSpecializedComponent('goals');
...
describe('Permissions Flow - Actions based', () => {
  it('should show only modules user has access to in navigation', async () => {
    await loginAsUser('limited@test.com'); // User con solo 'goals'

    await expect(page.locator('[data-nav="goals"]')).toBeVisible();
    await expect(page.locator('[data-nav="contributions"]')).not.toBeVisible();
  });

  it('should render CRUD module with actions from metadata', async () => {
    await loginAsUser('admin@test.com');
    await page.goto('/m/goals');
  ...
  it('SuperAdmin should access all modules with all actions', async () => {
    ...
    await expect(page.locator('[data-nav="goals"]')).toBeVisible();
    await expect(page.locator('[data-nav="contributions"]')).toBeVisible();
  ...
  it('should redirect unauthorized access to module', async () => {
    ...
    await page.goto('/m/contributions'); // No tiene este modulo
