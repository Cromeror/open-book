...
// components/client/objetivos-list.tsx
'use client';

import { usePermissions } from '@/hooks/usePermissions'; // NO!

export function ObjetivosList({ objetivos }) {
  ...
  return (
    <ul>
      {objetivos.map(obj => (
        <li key={obj.id}>
          {obj.nombre}
          {can('objetivos:delete') && <DeleteButton />} // ❌ INSEGURO
        </li>
      ))}
    </ul>
  );
...
// app/(dashboard)/objetivos/page.tsx (Server Component)
import { redirect } from 'next/navigation';
import { getServerPermissions } from '@/lib/permissions.server';
import { ObjetivosList } from '@/components/client/objetivos-list';
...
export default async function ObjetivosPage() {
  ...
  const objetivos = await fetchObjetivos();
 ...
 *   canUpdate: 'objetivos:update',
 *   canDelete: 'objetivos:delete',
 *   canExport: 'objetivos:export',
 * });
 *
...
const permissionProps = createPermissionProps(permissions, {
  canCreate: 'objetivos:create',
  canUpdate: 'objetivos:update',
  canDelete: 'objetivos:delete',
});

return <ObjetivosList objetivos={objetivos} {...permissionProps} />;
```
...
export default async function Page() {
  return (
    <div>
      ...
      <ShowForPermission permission="reportes:export">
        <ExportSection />
      </ShowForPermission>
    </div>
  );
