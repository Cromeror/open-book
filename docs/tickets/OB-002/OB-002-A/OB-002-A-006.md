...
- [ ] Campo `role` eliminado de la entidad User
- [ ] Enum `Role` eliminado del codigo
- [ ] Campo `isSuperAdmin` agregado (boolean, default false)
- [ ] Migracion creada para actualizar la tabla
- [ ] Migracion maneja datos existentes (convertir ADMIN a isSuperAdmin si aplica)
- [ ] Tests actualizados
- [ ] No hay referencias a Role.ADMIN o Role.RESIDENT en el codigo
- [ ] Response de registro (POST /api/auth/register) no incluye campo `role`
- [ ] DTOs de usuario actualizados sin campo `role`
...
- [ ] No hay errores de compilacion por referencias a Role
