...
export default async function CommitmentDetailPage({ params }: Props) {
  ...
  const canViewContributions = permissions.can('compromisos:read');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-900">Detalle del Compromiso</h1>
        <p className="text-gray-600">
          Informacion del compromiso #{id}
        </p>
      </div>
        ...
        <div className="text-center">
          ...
          <div className="mt-4 text-left text-xs text-gray-400">
            ...
            <ul className="mt-2 list-inside list-disc space-y-1">
              ...
              <li>Estado: Pendiente / Parcial / Cumplido</li>
              <li>Aportes que cumplen este compromiso</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      {canViewContributions && (
        <div className="flex gap-3">
          <Link
            href={`/commitments/${id}/contributions`}
            className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Ver Aportes Asociados
          </Link>
        </div>
      )}
    </div>
